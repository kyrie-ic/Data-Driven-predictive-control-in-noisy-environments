...
%% Recursive DeePC (with noise and convex combination + regularization)
for t_step = 1:p_steps
    ...
    % Apply input to system and simulate response
...
%% Recursive DeePC (with noise and convex combination)
x_new_convex = zeros(4, p_steps);
u_ini = repmat(zeros(m, 1), T_ini, 1);
y_ini = repmat(zeros(p, 1), T_ini, 1);

H_convex = 2 * (Y_f' * Q * Y_f);
H_convex = (H_convex + H_convex') / 2;
y_new_convex_all = [];
...
for t_step = 1:p_steps
    ...
    [g_opt, fval] = quadprog(H_convex, f(target_angle), A1, b1, Aeq, beq, [], [], [], options);
    
    u_opt = U_f * g_opt;
    y_opt = Y_f * g_opt;
    
    y_convex_all = [y_convex_all; y_opt(1:p)'];

    x_new_convex(:, t_step+1) = Ad * x_new_convex(:, t_step) + Bd * u_opt(1:m)';
    y_new_convex = Cd * x_new_convex(:, t_step) + Dd * u_opt(1:m)';
    y_new_convex_all = [y_new_convex_all; y_new_convex'];
    
    u_ini = [u_ini(m+1:end); u_opt(1:m)'];
    y_ini = [y_ini(p+1:end); y_new_convex];
    
end

%% Recursive DeePC (noise-free, no convex combination)
...
f1 = @(target_angle) -2 * (target_angle' * Q * Y_f_no_noise)';
...
for t_step = 1:p_steps
    ...
    [g_opt, fval] = quadprog(H_no_noise, f1(target_angle), [], [], Aeq, beq, [], [], [], options);
...
%% Plot results
figure;
plot(0:dt:(length(y_opt_all)-1)*dt, target_angel_all(:, 1), '--', 'LineWidth', 1, 'DisplayName', 'Reference');
hold on;
plot(0:dt:(length(y_opt_all)-1)*dt, y_new_all(:, 1), 'LineWidth', 1, 'DisplayName', 'Proposed framework with regularization');
plot(0:dt:(length(y_opt_all)-1)*dt, y_new_convex_all(:, 1), 'LineWidth', 1, 'DisplayName', 'Basic proposed framework');
